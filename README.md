...
- 用户统计层：按用户聚合 GPU 使用时长、非空闲时长与平均利用率，支持 1 / 3 / 7 / 14 / 30 / 90 / 180 / 365 天窗口，以及自定义起止日期。
- 采样策略：默认每 10 分钟通过 SSH 执行 `nvidia-smi`，只保留每日聚合结果。GPU 日聚合默认保留 60 天；用户日聚合默认保留 400 天，以支撑 90 / 180 / 365 天与自定义窗口。
...
- 用户统计层的时间范围下拉框提供 `1 day` / `3 days` / `7 days` / `14 days` / `30 days` / `90 days` / `180 days` / `365 days`，以及 `Custom range`。选择 `Custom range` 后会出现起止日期输入框，点击 `Apply` 才会发起查询。
