...
GPU Monitor 是一个面向实验室 GPU 集群的 Web 管理面板：用户可先通过 `ssh -L 8000:127.0.0.1:8000 username@10.193.104.165`（或通过 VS Code Remote SSH）连到部署机，再在本地浏览器访问 `http://127.0.0.1:8000` 打开页面；首次登录必须填写邮箱（后续可不填，若填写新邮箱会更新），并输入自己的 SSH 账号信息以验证可访问服务器。页面主要包含当前状态层（每台机器 GPU 的 util / 显存 / 活跃用户）、历史统计层（7/14/20/30 天利用率与占用趋势）、用户统计层（用户使用时长聚合），以及低利用率待处理任务面板。系统规则包括：当检测到低利用率任务（util < 40%）会发送邮件到用户邮箱，邮件里会要求填写说明表单；未申请延期时默认 4 小时后自动 kill，用户可申请 4/8/12/24 小时延期并填写原因，延期结束后仍会执行 kill；管理员（lifu / panzhou）可查看全部待 kill 任务并直接手动 kill。 
