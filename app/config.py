...
class Settings(BaseSettings):
    ...
    @property
    def host_gpu_type_map(self) -> dict[str, str]:
        mapping: dict[str, str] = {}
        for pair in self.host_gpu_types.split(','):
            item = pair.strip()
            if not item or ':' not in item:
                continue
            address, gpu_type = item.split(':', 1)
            address = address.strip()
            gpu_type = gpu_type.strip()
            if address and gpu_type:
                mapping[address] = gpu_type
        return mapping
