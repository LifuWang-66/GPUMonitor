...
from app.models import Host, KillCandidate, UserProfile
from app.schemas import (
    CredentialCheckRequest,
    HostAccessResult,
    KillCandidateResponse,
    KillExtensionRequest,
    SessionResponse,
    TestEmailRequest,
    TestEmailResponse,
...
from app.services.ssh_client import SshCredentials, close_collector_connections, fetch_home_users, kill_user_gpu_processes, validate_host_access

settings = get_settings()
scheduler = BackgroundScheduler(timezone='UTC')
ADMIN_USERNAMES = {'lifu', 'panzhou'}
ALLOWED_EXTENSION_HOURS = {4, 8, 12, 24}
...
def _candidate_visible_hosts(viewer: str, allowed_hosts: list[str]) -> list[str]:
    if viewer in ADMIN_USERNAMES:
        return [item['address'] for item in settings.hosts]
...
@app.get('/api/kill-candidates', response_model=list[KillCandidateResponse])
def api_kill_candidates(request: Request, allowed_hosts: list[str] = Depends(get_allowed_hosts), db: Session = Depends(get_db)):
    viewer = (request.session.get('username') or '').strip()
    if not viewer:
        raise HTTPException(status_code=401, detail='Please login first.')
    visible_hosts = _candidate_visible_hosts(viewer, allowed_hosts)
    if not visible_hosts:
        return []

    rows = db.execute(
        select(KillCandidate, Host)
        .join(Host, KillCandidate.host_id == Host.id)
        .where(
            Host.address.in_(visible_hosts),
            KillCandidate.killed_at.is_(None),
        )
        .order_by(KillCandidate.first_detected_at.asc())
    ).all()

    now = datetime.utcnow()
    items: list[KillCandidateResponse] = []
    for candidate, host in rows:
        if viewer not in ADMIN_USERNAMES and candidate.username != viewer:
            continue
        kill_after_at = candidate.extension_expires_at or (candidate.first_detected_at + timedelta(hours=4))
        total_hours = max((now - candidate.first_detected_at).total_seconds() / 3600, 0.0)
        items.append(
            KillCandidateResponse(
                id=candidate.id,
                ...
                utilization_gpu=round(candidate.last_utilization, 2),
                memory_used_mb=round(candidate.last_memory_used_mb, 2),
                first_detected_at=candidate.first_detected_at,
                last_seen_at=candidate.last_seen_at,
                kill_after_at=kill_after_at,
                extension_hours=candidate.extension_hours,
                extension_reason=candidate.extension_reason,
                extension_expires_at=candidate.extension_expires_at,
...
@app.post('/api/kill-candidates/{candidate_id}/extension', response_model=KillCandidateResponse)
def api_kill_candidate_extension(
    candidate_id: int,
    payload: KillExtensionRequest,
    request: Request,
    ...
    if not viewer:
        raise HTTPException(status_code=401, detail='Please login first.')
    if payload.hours not in ALLOWED_EXTENSION_HOURS:
        raise HTTPException(status_code=400, detail='hours must be one of: 4, 8, 12, 24')

    candidate_row = db.execute(
        select(KillCandidate, Host).join(Host, KillCandidate.host_id == Host.id).where(KillCandidate.id == candidate_id, KillCandidate.killed_at.is_(None))
    ).first()
    if not candidate_row:
        raise HTTPException(status_code=404, detail='Kill candidate not found.')

    candidate, host = candidate_row
    visible_hosts = _candidate_visible_hosts(viewer, allowed_hosts)
    if host.address not in visible_hosts:
        raise HTTPException(status_code=403, detail='No permission for this host.')
    if viewer not in ADMIN_USERNAMES and candidate.username != viewer:
        raise HTTPException(status_code=403, detail='You can only request extension for your own jobs.')

    now = datetime.utcnow()
    candidate.extension_hours = payload.hours
    candidate.extension_reason = payload.reason.strip()
    candidate.extension_requested_at = now
    candidate.extension_expires_at = now + timedelta(hours=payload.hours)
    db.commit()
    total_hours = max((now - candidate.first_detected_at).total_seconds() / 3600, 0.0)
    return KillCandidateResponse(
        id=candidate.id,
        username=candidate.username,
        host_name=host.name,
        host_address=host.address,
...
@app.post('/api/kill-candidates/{candidate_id}/kill', response_model=KillCandidateResponse)
def api_kill_candidate_now(
    candidate_id: int,
    request: Request,
    allowed_hosts: list[str] = Depends(get_allowed_hosts),
    db: Session = Depends(get_db),
    ...
    if viewer not in ADMIN_USERNAMES:
        raise HTTPException(status_code=403, detail='Only lifu and panzhou can kill directly.')
    ...
    if credentials is None:
        raise HTTPException(status_code=500, detail='Collector credentials are not configured.')

    kill_user_gpu_processes(host.address, credentials, candidate.username)
    now = datetime.utcnow()
    candidate.killed_at = now
    candidate.kill_triggered_by = viewer
    db.commit()
    total_hours = max((now - candidate.first_detected_at).total_seconds() / 3600, 0.0)
    return KillCandidateResponse(
        id=candidate.id,
