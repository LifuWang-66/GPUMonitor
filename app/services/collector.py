...
from app.models import CurrentGpuStatus, DailyGpuAggregate, DailyUserAggregate, Host, KillCandidate, NotificationEvent, UserProfile, UserUtilizationSample
...
def _evaluate_and_handle_user_alerts(db: Session, host: Host, snapshot: HostSnapshot, credentials: SshCredentials) -> None:
    ...
    per_user_memory_used_mb: dict[str, float] = {}
    for record in snapshot.gpu_records:
        for username in record.active_users:
            if username in settings.excluded_users:
                continue
            per_user_gpu_count[username] = per_user_gpu_count.get(username, 0) + 1
            per_user_memory_used_mb[username] = per_user_memory_used_mb.get(username, 0.0) + record.memory_used_mb
    ...
    for username, gpu_count in per_user_gpu_count.items():
        ...
        if LOW_UTIL_THRESHOLD <= four_hour_max <= MID_UTIL_THRESHOLD:
            ...
            kill_result = _upsert_kill_candidate_and_process(
                db=db,
                host=host,
                username=username,
                now=snapshot.collected_at.replace(tzinfo=None),
                max_utilization=four_hour_max,
                memory_used_mb=per_user_memory_used_mb.get(username, 0.0),
                credentials=credentials,
                kill_triggered_by='system',
            )
            if kill_result == 'pending':
                action_text = 'The job is queued for kill after 4 hours unless an extension is approved.'
            else:
                action_text = f'GPU processes were terminated. Killed PIDs: {kill_result or "none"}'
            _notify_issue_with_escalation(
                db,
                host,
                username,
            ...
            _clear_kill_candidate(db, host.id, username)

    active_usernames = set(per_user_gpu_count.keys())
    _clear_absent_kill_candidates(db, host.id, active_usernames)
...
def _upsert_kill_candidate_and_process(
    db: Session,
    host: Host,
    username: str,
    now: datetime,
    ...
    kill_triggered_by: str,
) -> str:
    candidate = db.scalar(
        select(KillCandidate).where(
            KillCandidate.host_id == host.id,
            KillCandidate.username == username,
    ...
    if candidate is None:
        candidate = KillCandidate(
            ...
            last_seen_at=now,
            last_utilization=max_utilization,
            last_memory_used_mb=memory_used_mb,
        )
        db.add(candidate)
        return 'pending'

    candidate.last_seen_at = now
    candidate.last_utilization = max_utilization
    candidate.last_memory_used_mb = memory_used_mb

    deadline = candidate.extension_expires_at or (candidate.first_detected_at + KILL_GRACE_PERIOD)
    if now < deadline:
        return 'pending'

    if candidate.killed_at is not None:
        return 'none'

    kill_result = kill_user_gpu_processes(host.address, credentials, username)
    candidate.killed_at = now
    candidate.kill_triggered_by = kill_triggered_by
    return kill_result or 'none'


def _clear_kill_candidate(db: Session, host_id: int, username: str) -> None:
    candidate = db.scalar(
        select(KillCandidate).where(
            KillCandidate.host_id == host_id,
            KillCandidate.username == username,
            KillCandidate.killed_at.is_(None),
        )
    )
...
def _clear_absent_kill_candidates(db: Session, host_id: int, active_usernames: set[str]) -> None:
    ...
    for candidate in candidates:
        if candidate.username not in active_usernames:
            db.delete(candidate)
