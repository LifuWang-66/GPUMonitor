...
function createServerSection(hostName, hostAddress, cards) {
  ...
  section.innerHTML = `
    <div class="server-section-head">
      ...
      <div class="server-summary-badge">${cards.length} 张 GPU</div>
    </div>
...
function renderCurrent(cards) {
  ...
  for (const group of Object.values(grouped)) {
    const section = createServerSection(group.hostName, group.hostAddress, group.items);
...
function renderUsers(items) {
  ...
  for (const item of items) {
    ...
    row.innerHTML = `
      <td>${item.username}</td>
      <td>${item.host_names.join(', ')}<div class="table-subtext">${item.host_addresses.join(', ')}</div></td>
