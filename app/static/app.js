...
const killCandidateWrapper = document.getElementById('kill-candidate-wrapper');
const windowSelect = document.getElementById('window-select');
const refreshButton = document.getElementById('refresh-button');
const logoutButton = document.getElementById('logout-button');
const adminUsers = new Set(['lifu', 'panzhou']);
...
function renderKillCandidates(items) {
  if (!killCandidateWrapper) return;
  killCandidateWrapper.innerHTML = '';
  if (!items.length) {
    killCandidateWrapper.textContent = 'No pending kill candidates.';
    killCandidateWrapper.classList.add('empty-state');
    return;
  }
  killCandidateWrapper.classList.remove('empty-state');
  const isAdmin = adminUsers.has((bootstrap.sessionUsername || '').trim());
  ...
  for (const item of items) {
    const block = document.createElement('article');
    block.className = 'user-card';
    const extensionText = item.extension_hours
      ? `${item.extension_hours}h until ${formatDateTime(item.extension_expires_at)}`
      : 'No extension';
    const reasonText = item.extension_reason || '--';
    block.innerHTML = `
      <div class="user-card-head">
        <div>
          <h3>${item.username}</h3>
          <p class="muted">${item.host_name} · ${item.host_address}</p>
        </div>
        <div class="user-summary-list">
          <span class="server-summary-badge">Util: ${item.utilization_gpu}%</span>
          <span class="server-summary-badge">Memory: ${item.memory_used_mb} MB</span>
          <span class="server-summary-badge">Running: ${item.total_running_hours} h</span>
          <span class="server-summary-badge">Kill after: ${formatDateTime(item.kill_after_at)}</span>
        </div>
      </div>
      <div class="kill-meta">
        <p><strong>Extension:</strong> ${extensionText}</p>
        <p><strong>Reason:</strong> ${reasonText}</p>
      </div>
      <div class="kill-actions"></div>
    `;

    const actions = block.querySelector('.kill-actions');
    if (isAdmin) {
      const killButton = document.createElement('button');
      killButton.className = 'secondary';
      killButton.textContent = 'Kill now';
      killButton.addEventListener('click', async () => {
        killButton.disabled = true;
        try {
          await fetchJson(`/api/kill-candidates/${item.id}/kill`, { method: 'POST' });
          await refreshKillCandidates();
        } catch (error) {
      ...
      select.innerHTML = `
        <option value="4">Extend 4h</option>
        <option value="8">Extend 8h</option>
        <option value="12">Extend 12h</option>
        <option value="24">Extend 24h</option>
      `;
      const reasonInput = document.createElement('input');
      reasonInput.type = 'text';
      reasonInput.placeholder = 'Why do you need extension?';
      reasonInput.maxLength = 1000;
      const applyButton = document.createElement('button');
      applyButton.className = 'secondary';
      applyButton.textContent = 'Request extension';
      applyButton.addEventListener('click', async () => {
        ...
        try {
          await fetchJson(`/api/kill-candidates/${item.id}/extension`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ hours: Number(select.value), reason }),
          });
          await refreshKillCandidates();
        } catch (error) {
  ...
  killCandidateWrapper.appendChild(list);
}

async function refreshKillCandidates() {
  if (!bootstrap.accessibleHosts.length || !killCandidateWrapper) return;
  const items = await fetchJson('/api/kill-candidates');
  renderKillCandidates(items);
}
